...
constexpr size_t kMaxMarginalizedBacksubstitutions = 512U;
...
void SlidingWindowOptimizer::clear()
{
  ...
  relative_distance_factors_.clear();
  ...
  relative_distance_factor_replacement_count_ = 0U;
...
bool SlidingWindowOptimizer::add_marginalized_measurement_prior(
  const int64_t factor_stamp_ns,
  const Eigen::VectorXd & residual,
  const Eigen::MatrixXd & marginalized_jacobian)
{
  if (residual.size() == 0 || marginalized_jacobian.rows() != residual.size() ||
    marginalized_jacobian.cols() != static_cast<Eigen::Index>(kStateDof) ||
    !residual.allFinite() || !marginalized_jacobian.allFinite())
  {
    return false;
  }
  ...
  const bool added = add_dense_measurement_prior(
    support->retained_stamp_ns,
    support->retained_reference_states,
    retained_residual,
    retained_jacobian);
  if (added && support->interpolated) {
...
{
  if (retained_stamp_ns.empty() ||
    retained_stamp_ns.size() != retained_reference_states.size() ||
    jacobian.rows() != residual.size() ||
    jacobian.cols() != static_cast<Eigen::Index>(retained_stamp_ns.size() * kStateDof) ||
    !residual.allFinite() ||
    !jacobian.allFinite())
  {
    return false;
  }
  for (const auto stamp_ns : retained_stamp_ns) {
    if (find_state_index(stamp_ns) < 0) {
      return false;
    }
  }
  Eigen::MatrixXd retained_jacobian = jacobian;
  ...
  if (retained_jacobian.norm() <= 1.0e-12 || !residual.allFinite() ||
    !retained_jacobian.allFinite())
  {
    return false;
  }

  const Eigen::MatrixXd hessian = retained_jacobian.transpose() * retained_jacobian;
  const Eigen::VectorXd rhs = -retained_jacobian.transpose() * residual;
  const Eigen::MatrixXd sqrt_information = sqrt_information_from_hessian(hessian);
  if (sqrt_information.rows() == 0 || !sqrt_information.allFinite()) {
    return false;
  }
  const Eigen::MatrixXd regularized =
    hessian + config_.damping * Eigen::MatrixXd::Identity(hessian.rows(), hessian.cols());
  const Eigen::VectorXd target_delta = regularized.ldlt().solve(rhs);
  if (!target_delta.allFinite()) {
    return false;
  }

  SlidingWindowDensePrior prior;
  prior.stamp_ns = retained_stamp_ns;
  prior.reference_states = retained_reference_states;
...
std::optional<SlidingWindowOptimizer::MeasurementEndpointBacksubstitution>
SlidingWindowOptimizer::build_measurement_endpoint_backsubstitution(
  const int64_t factor_stamp_ns) const
{
  const int index = find_state_index(factor_stamp_ns);
  if (index >= 0) {
    MeasurementEndpointBacksubstitution endpoint;
    endpoint.reference_state = states_[static_cast<size_t>(index)];
    endpoint.retained_stamp_ns = {endpoint.reference_state.stamp_ns};
    endpoint.retained_reference_states = {endpoint.reference_state};
    endpoint.target_delta.setZero();
    endpoint.delta_from_retained =
  ...
  const auto marginalized = build_marginalized_measurement_backsubstitution(factor_stamp_ns);
  if (!marginalized.has_value()) {
    return std::nullopt;
  }
  MeasurementEndpointBacksubstitution endpoint;
  endpoint.reference_state = marginalized->marginalized_reference_state;
  endpoint.retained_stamp_ns = marginalized->retained_stamp_ns;
  endpoint.retained_reference_states = marginalized->retained_reference_states;
  endpoint.target_delta = marginalized->marginalized_target_delta;
  endpoint.delta_from_retained = marginalized->marginalized_delta_from_retained;
  endpoint.interpolated = marginalized->interpolated;
  return endpoint;
}

bool SlidingWindowOptimizer::add_marginalized_relative_translation_prior(
  const SlidingWindowRelativeTranslationFactor & factor)
{
  if (factor.from_stamp_ns >= factor.to_stamp_ns ||
    !std::isfinite(factor.weight) || !std::isfinite(factor.huber_delta_m) ||
    ...
    !factor.delta_p_w.allFinite() ||
    !quaternion_is_finite_and_nonzero(factor.delta_q_from_to))
  {
    return false;
  }

  const auto from_endpoint =
    build_measurement_endpoint_backsubstitution(factor.from_stamp_ns);
  const auto to_endpoint =
    build_measurement_endpoint_backsubstitution(factor.to_stamp_ns);
  if (!from_endpoint.has_value() || !to_endpoint.has_value()) {
    return false;
  }
  if (from_endpoint->active && to_endpoint->active) {
    return false;
  }

  const auto from_q = from_endpoint->reference_state.q_w_i.normalized();
  const auto to_q = to_endpoint->reference_state.q_w_i.normalized();
  if (!from_endpoint->reference_state.p_w_i.allFinite() ||
    !to_endpoint->reference_state.p_w_i.allFinite() ||
    !quaternion_is_finite_and_nonzero(from_q) ||
    !quaternion_is_finite_and_nonzero(to_q))
  {
    return false;
  }

  const Eigen::Matrix3d r_from_w = from_q.inverse().toRotationMatrix();
  const Eigen::Vector3d delta_p_w =
    to_endpoint->reference_state.p_w_i - from_endpoint->reference_state.p_w_i;
  const Eigen::Vector3d predicted_delta_p =
    factor.translation_in_from_frame ? r_from_w * delta_p_w : delta_p_w;
  const Eigen::Vector3d translation_residual = predicted_delta_p - factor.delta_p_w;
  const double translation_scale =
    std::sqrt(factor.weight * huber_weight(translation_residual.norm(), factor.huber_delta_m));
  const bool include_rotation = factor.rotation_weight > 0.0;
  const Eigen::Index residual_rows = include_rotation ? 6 : 3;
  Eigen::VectorXd residual = Eigen::VectorXd::Zero(residual_rows);
  Eigen::MatrixXd from_jacobian =
    Eigen::MatrixXd::Zero(residual_rows, static_cast<Eigen::Index>(kStateDof));
  Eigen::MatrixXd to_jacobian =
    Eigen::MatrixXd::Zero(residual_rows, static_cast<Eigen::Index>(kStateDof));
  residual.segment<3>(0) = translation_scale * translation_residual;
  if (factor.translation_in_from_frame) {
    from_jacobian.block<3, 3>(0, 0) =
      translation_scale * r_from_w * skew_symmetric(delta_p_w);
    from_jacobian.block<3, 3>(0, 6) =
      -translation_scale * r_from_w;
    to_jacobian.block<3, 3>(0, 6) =
      translation_scale * r_from_w;
  } else {
    from_jacobian.block<3, 3>(0, 6) =
      -translation_scale * Eigen::Matrix3d::Identity();
    to_jacobian.block<3, 3>(0, 6) =
      translation_scale * Eigen::Matrix3d::Identity();
  }

  if (include_rotation) {
    auto measured_delta_q = factor.delta_q_from_to.normalized();
    const Eigen::Quaterniond predicted_delta_q = from_q.inverse() * to_q;
    const Eigen::Vector3d rotation_error =
      SlidingWindowOptimizer::rotation_residual(measured_delta_q, predicted_delta_q);
    const double rotation_scale =
      std::sqrt(
      factor.rotation_weight *
      huber_weight(rotation_error.norm(), factor.rotation_huber_delta_rad));
    const Eigen::Matrix3d rotation_middle_jacobian =
      rotation_residual_middle_perturbation_jacobian(measured_delta_q, predicted_delta_q);
    residual.segment<3>(3) = rotation_scale * rotation_error;
    from_jacobian.block<3, 3>(3, 0) =
      rotation_scale * rotation_middle_jacobian * (-r_from_w);
    to_jacobian.block<3, 3>(3, 0) =
      rotation_scale * rotation_middle_jacobian * r_from_w;
  }

  std::vector<int64_t> retained_stamp_ns;
  std::vector<SlidingWindowState> retained_reference_states;
  Eigen::MatrixXd retained_jacobian = Eigen::MatrixXd::Zero(residual_rows, 0);
  Eigen::VectorXd retained_residual = residual;
  ...
  auto append_endpoint =
    [&append_or_accumulate_block, &retained_residual](
      const Eigen::MatrixXd & endpoint_jacobian,
      const MeasurementEndpointBacksubstitution & endpoint) {
      if (endpoint.delta_from_retained.cols() == 0 ||
        endpoint.delta_from_retained.rows() != static_cast<Eigen::Index>(kStateDof) ||
        endpoint.delta_from_retained.cols() !=
        static_cast<Eigen::Index>(endpoint.retained_stamp_ns.size() * kStateDof) ||
        endpoint.retained_stamp_ns.size() != endpoint.retained_reference_states.size())
  ...
  if (!append_endpoint(from_jacobian, from_endpoint.value()) ||
    !append_endpoint(to_jacobian, to_endpoint.value()))
  {
    return false;
  }
  const bool added = add_dense_measurement_prior(
    ...
    retained_residual,
    retained_jacobian);
  if (added && (from_endpoint->interpolated || to_endpoint->interpolated)) {
    ++marginalized_backsubstitution_interpolation_count_;
  }
  return added;
}

SlidingWindowBatchedMarginalizedPriorResult
SlidingWindowOptimizer::add_batched_marginalized_visual_priors(
...
{
  SlidingWindowBatchedMarginalizedPriorResult result;
  ...
  auto append_or_accumulate_local_block = [](
    std::vector<int64_t> & stamp_ns,
    std::vector<SlidingWindowState> & reference_states,
    Eigen::MatrixXd & jacobian,
    Eigen::VectorXd & residual,
    ...
    const Eigen::MatrixXd & block) {
      ...
      const auto existing_it = std::find(stamp_ns.begin(), stamp_ns.end(), stamp);
      if (existing_it == stamp_ns.end()) {
        const auto old_cols = jacobian.cols();
        jacobian.conservativeResize(
          jacobian.rows(), old_cols + static_cast<Eigen::Index>(kStateDof));
        jacobian.block(
          0, old_cols, jacobian.rows(), static_cast<Eigen::Index>(kStateDof)) = block;
        stamp_ns.push_back(stamp);
        reference_states.push_back(reference_state);
        return;
      }
      const auto existing_index = static_cast<size_t>(
        std::distance(stamp_ns.begin(), existing_it));
      const auto existing_offset = static_cast<Eigen::Index>(existing_index * kStateDof);
      const auto reference_offset =
        SlidingWindowOptimizer::state_delta(reference_state, reference_states[existing_index]);
      residual += block * reference_offset;
      jacobian.block(
        0, existing_offset, block.rows(), static_cast<Eigen::Index>(kStateDof)) += block;
    };

  auto build_from_endpoints = [&append_or_accumulate_local_block](
    const std::vector<MeasurementEndpointBacksubstitution> & endpoints,
    const Eigen::VectorXd & residual,
    const std::vector<Eigen::MatrixXd> & endpoint_jacobians)
    -> std::optional<LocalMeasurement> {
      if (endpoints.empty() || endpoints.size() != endpoint_jacobians.size() ||
        residual.size() == 0 || !residual.allFinite())
      {
        return std::nullopt;
      ...
      measurement.jacobian = Eigen::MatrixXd::Zero(residual.size(), 0);
      measurement.interpolated = endpoints.size() > 1U;
      for (size_t endpoint_index = 0U; endpoint_index < endpoints.size(); ++endpoint_index) {
        const auto & endpoint = endpoints[endpoint_index];
        const auto & endpoint_jacobian = endpoint_jacobians[endpoint_index];
        if (endpoint_jacobian.rows() != residual.size() ||
        ...
        if (endpoint.interpolated) {
          measurement.interpolated = true;
        }
        measurement.residual += endpoint_jacobian * endpoint.target_delta;
        for (size_t retained_index = 0U; retained_index < endpoint.retained_stamp_ns.size();
          ++retained_index)
        {
          const auto offset = static_cast<Eigen::Index>(retained_index * kStateDof);
          const Eigen::MatrixXd block =
            ...
            static_cast<Eigen::Index>(kStateDof),
            static_cast<Eigen::Index>(kStateDof));
          append_or_accumulate_local_block(
            measurement.stamp_ns,
            measurement.reference_states,
      ...
      if (!has_marginalized_endpoint || measurement.jacobian.cols() == 0 ||
        measurement.jacobian.norm() <= 1.0e-12)
      {
        return std::nullopt;
      }
      return measurement;
  ...
  auto build_visual_measurement =
    [this, &build_from_endpoints](const SlidingWindowVisualAlignmentFactor & factor)
    -> std::optional<LocalMeasurement> {
      if (!std::isfinite(factor.weight) || !std::isfinite(factor.meters_per_pixel) ||
        factor.weight <= 0.0 || factor.meters_per_pixel <= 0.0 ||
      ...
      if (!factor.support_stamp_ns.empty()) {
        ...
        target_xy.x() =
          reference_p_w_i.x() + factor.measured_shift_px.x() * factor.meters_per_pixel;
        target_xy.y() =
      ...
      measurement.jacobian =
        marginalized_jacobian * support->marginalized_delta_from_retained;
      measurement.interpolated = support->interpolated;
      if (measurement.jacobian.cols() == 0 || measurement.jacobian.norm() <= 1.0e-12) {
        return std::nullopt;
      }
      return measurement;
    };

  auto build_se3_measurement =
    [this, &build_from_endpoints](const SlidingWindowSe3PhotometricFactor & factor)
    -> std::optional<LocalMeasurement> {
      if (!std::isfinite(factor.weight) || factor.weight <= 0.0 ||
  ...
  std::vector<int64_t> retained_stamp_ns;
  std::vector<SlidingWindowState> retained_reference_states;
  Eigen::VectorXd batched_residual = Eigen::VectorXd::Zero(0);
  Eigen::MatrixXd batched_jacobian = Eigen::MatrixXd::Zero(0, 0);
  size_t interpolated_measurement_count = 0U;

  auto append_measurement = [&](
    const LocalMeasurement & measurement) {
      if (measurement.stamp_ns.empty() ||
        measurement.stamp_ns.size() != measurement.reference_states.size() ||
        measurement.jacobian.rows() != measurement.residual.size() ||
        measurement.jacobian.cols() !=
        static_cast<Eigen::Index>(measurement.stamp_ns.size() * kStateDof) ||
        !measurement.residual.allFinite() ||
        !measurement.jacobian.allFinite())
      {
        return false;
      }
      const auto local_rows = measurement.residual.size();
      Eigen::VectorXd adjusted_residual = measurement.residual;
      std::vector<size_t> global_indices;
      global_indices.reserve(measurement.stamp_ns.size());
      for (size_t local_index = 0U; local_index < measurement.stamp_ns.size(); ++local_index) {
        const auto block_offset = static_cast<Eigen::Index>(local_index * kStateDof);
        const Eigen::MatrixXd block =
          measurement.jacobian.block(
          0, block_offset, local_rows, static_cast<Eigen::Index>(kStateDof));
        const auto existing_it =
          std::find(retained_stamp_ns.begin(), retained_stamp_ns.end(), measurement.stamp_ns[local_index]);
        if (existing_it == retained_stamp_ns.end()) {
          const auto old_cols = batched_jacobian.cols();
          batched_jacobian.conservativeResize(
            batched_jacobian.rows(), old_cols + static_cast<Eigen::Index>(kStateDof));
          if (batched_jacobian.rows() > 0) {
            batched_jacobian.block(
              0,
              old_cols,
              batched_jacobian.rows(),
              static_cast<Eigen::Index>(kStateDof)).setZero();
          }
          retained_stamp_ns.push_back(measurement.stamp_ns[local_index]);
          retained_reference_states.push_back(measurement.reference_states[local_index]);
          global_indices.push_back(retained_stamp_ns.size() - 1U);
          continue;
        }
        const auto existing_index = static_cast<size_t>(
          std::distance(retained_stamp_ns.begin(), existing_it));
        const auto reference_offset =
          SlidingWindowOptimizer::state_delta(
          measurement.reference_states[local_index],
          retained_reference_states[existing_index]);
        adjusted_residual += block * reference_offset;
        global_indices.push_back(existing_index);
      }

      const auto old_rows = batched_residual.size();
      const auto new_rows = old_rows + local_rows;
      const auto global_cols = batched_jacobian.cols();
      batched_residual.conservativeResize(new_rows);
      batched_residual.segment(old_rows, local_rows) = adjusted_residual;
      batched_jacobian.conservativeResize(new_rows, global_cols);
      batched_jacobian.block(old_rows, 0, local_rows, global_cols).setZero();
      for (size_t local_index = 0U; local_index < measurement.stamp_ns.size(); ++local_index) {
        const auto local_offset = static_cast<Eigen::Index>(local_index * kStateDof);
        const auto global_offset = static_cast<Eigen::Index>(global_indices[local_index] * kStateDof);
        batched_jacobian.block(
          old_rows, global_offset, local_rows, static_cast<Eigen::Index>(kStateDof)) +=
          measurement.jacobian.block(
          0, local_offset, local_rows, static_cast<Eigen::Index>(kStateDof));
      }
      if (measurement.interpolated) {
        ++interpolated_measurement_count;
      }
      return true;
    };

  for (const auto & visual_factor : visual_factors) {
    const auto measurement = build_visual_measurement(visual_factor);
    if (measurement.has_value() && append_measurement(measurement.value())) {
      ++result.visual_alignment_prior_count;
    } else {
      ++result.skipped_factor_count;
    }
  }
  for (const auto & se3_factor : se3_factors) {
    const auto measurement = build_se3_measurement(se3_factor);
    if (measurement.has_value() && append_measurement(measurement.value())) {
  ...
  if (result.skipped_factor_count > 0U ||
    result.visual_alignment_prior_count + result.se3_photometric_prior_count == 0U ||
    batched_jacobian.cols() == 0 || batched_jacobian.norm() <= 1.0e-12)
  {
    result.skipped_factor_count = visual_factors.size() + se3_factors.size();
    result.visual_alignment_prior_count = 0U;
    result.se3_photometric_prior_count = 0U;
    result.added = false;
    return result;
  }

  std::vector<size_t> order;
  order.reserve(retained_stamp_ns.size());
  for (size_t index = 0U; index < retained_stamp_ns.size(); ++index) {
    order.push_back(index);
  }
  std::sort(
    order.begin(), order.end(),
    [&retained_stamp_ns](const size_t lhs, const size_t rhs) {
      return retained_stamp_ns[lhs] < retained_stamp_ns[rhs];
    });
  std::vector<int64_t> sorted_stamp_ns;
  std::vector<SlidingWindowState> sorted_reference_states;
  sorted_stamp_ns.reserve(order.size());
  sorted_reference_states.reserve(order.size());
  Eigen::MatrixXd sorted_jacobian =
    Eigen::MatrixXd::Zero(
    batched_jacobian.rows(),
    static_cast<Eigen::Index>(order.size() * kStateDof));
  for (size_t sorted_index = 0U; sorted_index < order.size(); ++sorted_index) {
    const size_t original_index = order[sorted_index];
    sorted_stamp_ns.push_back(retained_stamp_ns[original_index]);
    sorted_reference_states.push_back(retained_reference_states[original_index]);
    sorted_jacobian.block(
      0,
      static_cast<Eigen::Index>(sorted_index * kStateDof),
      batched_jacobian.rows(),
      static_cast<Eigen::Index>(kStateDof)) =
      batched_jacobian.block(
      0,
      static_cast<Eigen::Index>(original_index * kStateDof),
      batched_jacobian.rows(),
      static_cast<Eigen::Index>(kStateDof));
  }

  result.added = add_dense_measurement_prior(
    sorted_stamp_ns,
  ...
  visual_marginalization_prior_count_ += result.visual_alignment_prior_count;
  se3_photometric_marginalization_prior_count_ += result.se3_photometric_prior_count;
  marginalized_backsubstitution_interpolation_count_ += interpolated_measurement_count;
  return result;
}

bool SlidingWindowOptimizer::add_marginalized_visual_alignment_prior(
...
bool SlidingWindowOptimizer::add_marginalized_se3_photometric_prior(
  const SlidingWindowSe3PhotometricFactor & factor)
{
  if (!std::isfinite(factor.weight) || factor.weight <= 0.0 ||
    !std::isfinite(factor.huber_delta) || factor.huber_delta < 0.0 ||
    !factor.target_delta.allFinite() || !factor.sqrt_information.allFinite() ||
...
void SlidingWindowOptimizer::add_relative_distance_factor(
  const SlidingWindowRelativeDistanceFactor & factor)
{
  if (factor.from_stamp_ns >= factor.to_stamp_ns) {
    throw std::runtime_error("relative distance factor timestamps must be strictly increasing");
  }
  if (!std::isfinite(factor.distance_m) || !std::isfinite(factor.weight) ||
    !std::isfinite(factor.huber_delta_m) ||
    factor.distance_m < 0.0 || factor.weight <= 0.0 || factor.huber_delta_m < 0.0)
  {
    throw std::runtime_error(
            "relative distance factor distance/weight must be finite and valid");
  }
  const auto existing = std::find_if(
    relative_distance_factors_.begin(), relative_distance_factors_.end(),
    [&factor](const SlidingWindowRelativeDistanceFactor & candidate) {
      return candidate.from_stamp_ns == factor.from_stamp_ns &&
             candidate.to_stamp_ns == factor.to_stamp_ns &&
             candidate.source_id == factor.source_id;
    });
  if (existing == relative_distance_factors_.end()) {
    relative_distance_factors_.push_back(factor);
  } else {
    *existing = factor;
    ++relative_distance_factor_replacement_count_;
  }
}
...
size_t SlidingWindowOptimizer::enforce_window_size()
{
  ...
  while (states_.size() > config_.max_states) {
    ...
    relative_distance_factors_.erase(
      std::remove_if(
        relative_distance_factors_.begin(), relative_distance_factors_.end(),
        [stamp_ns](const SlidingWindowRelativeDistanceFactor & factor) {
          return factor.from_stamp_ns == stamp_ns || factor.to_stamp_ns == stamp_ns;
        }),
      relative_distance_factors_.end());
  ...
  if (marginalized > 0U) {
    ...
    prune_marginalized_backsubstitutions();
  }
  return marginalized;
}
...
size_t SlidingWindowOptimizer::prune_marginalized_factor_references()
{
  ...
  removed += erase_and_count(
    ...
    [before_active_window](const SlidingWindowRelativeDistanceFactor & factor) {
      return before_active_window(factor.from_stamp_ns) ||
             before_active_window(factor.to_stamp_ns);
    });
  removed += erase_and_count(
  ...
  SlidingWindowCostBreakdown * breakdown) const
{
  ...
  for (const auto & factor : relative_distance_factors_) {
    const int from = find_local(factor.from_stamp_ns);
    const int to = find_local(factor.to_stamp_ns);
    if (from < 0 || to < 0) {
      continue;
    ...
    const Eigen::Vector3d delta_p =
      states[static_cast<size_t>(to)].p_w_i - states[static_cast<size_t>(from)].p_w_i;
    const double predicted_distance = delta_p.norm();
    const double residual_value = predicted_distance - factor.distance_m;
    const double robust_weight = huber_weight(std::abs(residual_value), factor.huber_delta_m);
    Eigen::VectorXd residual(1);
    residual[0] = std::sqrt(factor.weight * robust_weight) * residual_value;
    append(residual, &SlidingWindowCostBreakdown::relative_distance_factor_cost);
  }
  ...
  Eigen::MatrixXd & jacobian) const
{
  ...
  for (const auto & factor : relative_distance_factors_) {
    ...
    const double residual_value = predicted_distance - factor.distance_m;
    const double scale = std::sqrt(
      factor.weight * huber_weight(std::abs(residual_value), factor.huber_delta_m));
    if (predicted_distance > 1.0e-12) {
      const Eigen::RowVector3d direction = delta_p.transpose() / predicted_distance;
      const Eigen::Index from_offset = variable_offsets[static_cast<size_t>(from)];
      const Eigen::Index to_offset = variable_offsets[static_cast<size_t>(to)];
      if (from_offset >= 0) {
        jacobian.template block<1, 3>(row, from_offset + 6) = -scale * direction;
      }
      if (to_offset >= 0) {
        jacobian.template block<1, 3>(row, to_offset + 6) = scale * direction;
      }
    } else {
      mark_numeric(row, 1, 0, jacobian.cols());
    }
    ++row;
  }
...
bool SlidingWindowOptimizer::add_schur_marginalization_prior_for_front()
{
  ...
  const Eigen::VectorXd rhs_m = normal.rhs.segment(marginalized_offset, marginalized_dim);
  ...
  const Eigen::MatrixXd h_mm_regularized =
    h_mm + config_.damping * Eigen::MatrixXd::Identity(marginalized_dim, marginalized_dim);
  ...
  if (h_mm_solver.info() == Eigen::Success) {
    ...
    record.retained_stamp_ns.reserve(retained_variables.size());
    record.retained_reference_states.reserve(retained_variables.size());
    for (const auto & variable : retained_variables) {
      record.retained_stamp_ns.push_back(states_[variable.state_index].stamp_ns);
      record.retained_reference_states.push_back(states_[variable.state_index]);
    }
...
const SlidingWindowOptimizer::MarginalizedStateBacksubstitution *
SlidingWindowOptimizer::select_marginalized_backsubstitution(const int64_t factor_stamp_ns) const
{
  const MarginalizedStateBacksubstitution * best = nullptr;
  ...
  if (best == nullptr || best_delta_ns > max_delta_ns) {
    return nullptr;
  }
  return best;
}
  ...
  const int64_t factor_stamp_ns) const
{
  ...
  const SlidingWindowState * active_after = nullptr;
  for (const auto & state : states_) {
    if (state.stamp_ns <= factor_stamp_ns) {
      continue;
    }
    if (active_after == nullptr || state.stamp_ns < active_after->stamp_ns) {
      active_after = &state;
    }
  }
  if (before != nullptr && active_after != nullptr) {
    const int64_t span_ns = active_after->stamp_ns - before->marginalized_stamp_ns;
    const int64_t before_delta_ns = factor_stamp_ns - before->marginalized_stamp_ns;
    const int64_t after_delta_ns = active_after->stamp_ns - factor_stamp_ns;
    if (span_ns > 0 && before_delta_ns >= 0 && after_delta_ns >= 0 &&
      before_delta_ns <= max_delta_ns && after_delta_ns <= max_delta_ns)
    {
      const double alpha =
        std::clamp(static_cast<double>(before_delta_ns) / static_cast<double>(span_ns), 0.0, 1.0);
      MarginalizedMeasurementBacksubstitution output;
      ...
      output.marginalized_reference_state.p_w_i =
        (1.0 - alpha) * before->marginalized_reference_state.p_w_i +
        alpha * active_after->p_w_i;
      output.marginalized_reference_state.q_w_i =
        before->marginalized_reference_state.q_w_i.normalized().slerp(
        alpha, active_after->q_w_i.normalized()).normalized();
      output.marginalized_reference_state.v_w_i =
        (1.0 - alpha) * before->marginalized_reference_state.v_w_i +
        alpha * active_after->v_w_i;
      output.marginalized_reference_state.gyro_bias =
        (1.0 - alpha) * before->marginalized_reference_state.gyro_bias +
        alpha * active_after->gyro_bias;
      output.marginalized_reference_state.accel_bias =
        (1.0 - alpha) * before->marginalized_reference_state.accel_bias +
        alpha * active_after->accel_bias;
      output.marginalized_target_delta = (1.0 - alpha) * before->marginalized_target_delta;
      output.marginalized_delta_from_retained =
        Eigen::MatrixXd::Zero(static_cast<Eigen::Index>(kStateDof), 0);
      ...
      for (size_t retained_index = 0U; retained_index < before->retained_stamp_ns.size();
        ++retained_index)
      {
        const auto offset = static_cast<Eigen::Index>(retained_index * kStateDof);
        append_or_accumulate_block(
          before->retained_stamp_ns[retained_index],
          before->retained_reference_states[retained_index],
          (1.0 - alpha) *
          before->marginalized_delta_from_retained.block<15, 15>(0, offset));
      }
      append_or_accumulate_block(
        active_after->stamp_ns,
        *active_after,
        alpha * Eigen::Matrix<double, 15, 15>::Identity());
      if (!output.retained_stamp_ns.empty() &&
        output.marginalized_target_delta.allFinite() &&
        output.marginalized_delta_from_retained.allFinite())
      {
...
{
  const auto old_size = marginalized_backsubstitutions_.size();
  marginalized_backsubstitutions_.erase(
    std::remove_if(
      marginalized_backsubstitutions_.begin(), marginalized_backsubstitutions_.end(),
      [this](const MarginalizedStateBacksubstitution & record) {
        return std::any_of(
          record.retained_stamp_ns.begin(), record.retained_stamp_ns.end(),
          [this](const int64_t stamp_ns) {
            return find_state_index(stamp_ns) < 0;
    ...
    marginalized_backsubstitutions_.end());
  return old_size - marginalized_backsubstitutions_.size();
}
...
SlidingWindowSummary SlidingWindowOptimizer::optimize()
{
  ...
  summary.relative_distance_factor_count = relative_distance_factors_.size();
  ...
  summary.relative_translation_factor_replacement_count =
    relative_translation_factor_replacement_count_;
  summary.relative_distance_factor_replacement_count =
    relative_distance_factor_replacement_count_;
  ...
  auto refresh_cost_summary = [&summary, this]() {
      ...
      summary.relative_distance_factor_cost = breakdown.relative_distance_factor_cost;
