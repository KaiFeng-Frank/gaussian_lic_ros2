...
- Current 100% paper/super-paper completion audit: `scripts/check_paper_completion.py` reports `INCOMPLETE` with `28` blockers in `docs/paper_completion_report.md`. The blockers are not data availability issues; they are evidence-quality gaps: liveness-only continuous-time gates, a runtime-health native BA gate without reference RMSE, and the older meter-level native reference threshold that is looser than the 5 cm paper-grade target.
