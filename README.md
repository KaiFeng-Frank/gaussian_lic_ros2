...
- A default-off sliding-window position-velocity consistency residual now ties the finite-difference position rate to the optimized state velocity, giving the BA a real trajectory-shape residual instead of only second-difference smoothness. Local CBD 60 s sweeps confirm the residual is active but not yet a production default: weight `0.05` reduces path drift to `15.17%` but worsens RMSE to `1.081 m` at `/tmp/gaussian_feedback_posvel_smooth005_60s_probe/native_tracking_report.json`; weight `0.01` gives the best balance so far with RMSE `0.635 m`, `38.91%` coverage, and `23.14%` path drift at `/tmp/gaussian_feedback_posvel_smooth001_60s_probe/native_tracking_report.json`; weight `0.005` regresses to RMSE `1.059 m` at `/tmp/gaussian_feedback_posvel_smooth0005_60s_probe/native_tracking_report.json`. This is a useful new tuning axis, but the fixed `0.025 m` guard remains the documented Gaussian feedback preset until it beats the current `0.585 m` 60 s checkpoint and holds on full120.
