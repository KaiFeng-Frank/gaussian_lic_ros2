...
- Current 100% paper/super-paper completion audit: `scripts/check_paper_completion.py` reports `INCOMPLETE` with `14` blockers in `docs/paper_completion_report.md`. The FAST-LIVO2 `CBD_Building_01` native tracker evidence is sponsored by the hash-locked Coco-LIC2 cm-scale reference-parity gate. The remaining non-documentation blockers are now concrete trajectory-evidence failures: Retail and R3LIVE only have zero-translation ROS1 mapper baselines as candidate references, while the archived M2DGR/MCD continuous-time outputs diverge by orders of magnitude before a 5 cm reference comparison can pass.
