...
- Native reports now include a `visual_factor_continuity` block derived from the tracking-status bins. It records per-bin visual/SE3 factor deltas together with rendered/observed/depth miss and stale deltas, SE3 valid/rejected batch deltas, and saturated-alignment deltas, so future full-window runs can distinguish transport/freshness loss from visual quality rejection before another production-preset candidate is considered.
- Sliding-window factor `source_id` is now 64-bit internally. The previous 8-bit visual rendered/observed stamp hash could collide over long replay windows and make the optimizer replace distinct visual/SE3 observations that landed on the same reference stamp; widening keeps the existing default source IDs while removing that hidden factor-continuity failure mode.
