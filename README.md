...
- Gaussian-feedback strict replay now exposes mapper image input QoS controls through `--mapper-feedback-image-qos-reliability` and `--mapper-feedback-image-qos-depth`, while keeping the default at `best_effort`. A direct reliable-subscription trial is not promoted because the current rosbag playback offers image QoS in a way that leaves a reliable mapper subscriber with zero image frames; fixing this requires a compatible rosbag playback QoS override or subscriber compatibility path, not a blind default flip.
...
mapper image QoS. The direct reliable image-QoS trial is rejected because it
receives zero mapper images from this bag/player combination, so the next fix is
rosbag playback QoS compatibility or mapper-side subscription compatibility
rather than a default reliable image subscription.
