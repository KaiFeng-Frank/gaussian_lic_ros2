...
- Native tracking probes are registered with CTest, so `colcon test --packages-select gaussian_lic_tracking` runs trajectory, IMU, LiDAR, sliding-window, bias observability, geometric Jacobian, Gaussian snapshot, and visual checks automatically. `scripts/tracking_smoke_test.sh` also verifies the launch path through `/gaussian_lic/frontend/status`, including bias observability, visual alignment, and photometric linearization status.

Still pending:

- Full Coco-LIC2-grade frontend/tracking algorithm: production sliding-window joint BA, full IMU/camera photometric Jacobians, and production-scale marginalization policy.
...
- Full SE3 camera photometric pose Jacobians and full joint BA remain to be ported beyond the current deskewed bounded 6-DoF LiDAR correction, 2-DoF visual translation linearization gate, bias observability gate, and finite-difference IMU-window foundation.
