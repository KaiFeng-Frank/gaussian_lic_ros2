...
- Latest 2026-05-25 chained marginalized back-substitution: old marginalized visual-feedback back-substitution records now propagate through later Schur marginalizations instead of expiring as soon as one retained support state leaves the active window. When a retained state is marginalized again, the optimizer substitutes that state's `H_mm^-1(rhs_m - H_mr dr)` relation into every older record, merges duplicate retained stamps with reference-state offsets, and reports `sliding_window_marginalized_backsubstitutions` plus `sliding_window_marginalized_backsubstitution_chain_updates` in status and native reports. The deterministic `sliding_window_schur_marginalization_probe` now includes a chained late-visual case with `active_backsubs=2`, `chain_updates=1`, and a successful late marginalized visual prior after the original retained support state has left the window. This is a non-tuning global-information-retention change; full-window production promotion still requires bag evidence that the longer-lived late priors improve RMSE without over-trusting inconsistent rendered feedback.
