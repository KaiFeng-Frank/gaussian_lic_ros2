...
This repository is now an executable ROS2 porting checkpoint for the public Gaussian-LIC/Gaussian-LIC2 code path. It has native ROS2 message, launch, adapter, mapper, CUDA/Torch Gaussian backend plumbing, native tracking factors, artifact extraction, strict replay/readiness tooling, an official FAST-LIVO2 Bright substitute proof chain, local strict FAST-LIVO2 `CBD_Building_01`, MCD `ntu_day_01`, and R3LIVE `hku_park_00` reproduction chains against archived ROS1 upstream baselines, plus trusted upstream/native reference trajectory gates for the FAST-LIVO2 `CBD_Building_01` frontend, the continuous-time tracker producer chain, and a three-scene M2DGR faithful Coco-LIC tracking evidence gate. The archived strict mapper-contract/CUDA reports are green for trajectory, PSNR/SSIM/LPIPS, GT-associated render-pair, and point-cloud gates; the full local strict evidence matrix now reports `required=21/21` in a clean worktree.

It is **not a claim of universal super-paper performance on every possible sequence**: Retail, MCD, and R3LIVE still rely on real-bag liveness/producer-chain continuous-time gates instead of final RMSE-gated native tracker parity. The current release claim is narrower and executable: the required local strict parity matrix passes, including mapper-contract/CUDA parity, the FAST-LIVO2 `CBD_Building_01` native frontend comparison against an upstream Coco-LIC-generated reference, M2DGR room_01/room_02/room_03 faithful Coco-LIC port trajectory evidence, and continuous-time tracker evidence on FAST-LIVO2, MCD, and R3LIVE bags.
...
- `scripts/check_strict_parity_matrix.py` for the final full-dataset release gate. It now reports `required=21/21`: FAST-LIVO2 mapper strict parity, FAST-LIVO hku1/hku2/Visual_Challenge/LiDAR_Degenerate mapper strict parity, M2DGR room_01/room_02/room_03 mapper strict parity, M2DGR room_01/room_02/room_03 faithful Coco-LIC tracking parity, MCD `ntu_day_01` mapper strict parity, R3LIVE `hku_park_00` mapper strict parity, CBD native BA runtime health, CBD native reference trajectory parity, Coco-LIC2 port cm-scale reference parity with strict artifact hashes, and the remaining continuous-time native tracker producer-chain gates are all archived and passing.
- `scripts/check_paper_completion.py` for the stricter 100% paper/super-paper completion audit. The current archived report at `docs/paper_completion_report.md` is intentionally `INCOMPLETE`: it keeps the remaining non-paper-level native tracking gates machine-visible instead of letting the `21/21` release matrix be misread as full paper completion.
- `scripts/audit_strict_data_inputs.py` for the data-side gate. The current local audit is archived at `docs/strict_data_status.md` / `docs/strict_data_status.json`; it now separates raw/frontend data, ROS1 baseline artifacts, ROS2 current artifacts, and native reference trajectories. As of 2026-06-02 all required profiles pass those input checks and the strict evidence matrix is `21/21`.
...
- RMSE-gated continuous-time native tracker parity on the remaining full long-window datasets; the data and reference-trajectory archive gate itself is now complete.
- Current 100% paper/super-paper completion audit: `scripts/check_paper_completion.py` reports `INCOMPLETE` with `21` blockers in `docs/paper_completion_report.md` in a clean worktree. The FAST-LIVO2 `CBD_Building_01` native tracker evidence is sponsored by the hash-locked Coco-LIC2 cm-scale reference-parity gate, and M2DGR room_01/room_02/room_03 are sponsored by the hash-locked faithful Coco-LIC port evidence. The remaining non-documentation blockers are concrete trajectory-evidence failures: Retail and R3LIVE only have zero-translation ROS1 mapper baselines as candidate references, MCD still lacks a passing paper-level native tracker comparison, and stale continuous-time native reports now also fail when they predate the output-pose guard or lack self-described evidence paths.
...
| Full-dataset strict parity matrix | PASS: `scripts/check_strict_parity_matrix.py` reports `required=21/21` in a clean worktree with FAST-LIVO, FAST-LIVO2, M2DGR, MCD, and R3LIVE covered by required mapper/native/continuous-time evidence |
...
Current local matrix status: `PASS`, `required=21/21`, `covered_profiles=fastlivo,fastlivo2,m2dgr,mcd,r3live`.
...
parity, M2DGR room_01/room_02/room_03 faithful Coco-LIC tracking parity, MCD `ntu_day_01` mapper-contract/CUDA strict parity, R3LIVE
...
FAST-LIVO2 Retail, MCD ntu_day_01, and R3LIVE hku_park_00 as
liveness/producer-chain gates; M2DGR now has a separate three-room faithful-port
trajectory sponsor instead of relying on the invalid global 5 cm assumption.
