...
- Latest 2026-05-23 rendered-feedback full122 rejection evidence: source-stamped feedback alone does not close the strict gap. `rendered_source_pointcloud` under-drains at `946/1015` visual/SE3 with `64` stale drops and regresses to `1.509 m / 5.36%`; `rendered_source_image` improves only to `960/1015` with `62` stale drops and `1.492 m / 5.24%`; adding callback-time ingest regresses to `931/1015` and `1.626 m / 5.30%`. Expired-factor projection reaches `1047/1015` factors and zero stale drops, but over-constrains the active trajectory and regresses to `1.766 m / 8.02%`. These artifacts are archived under `results/fastlivo2/CBD_Building_01_rendered_feedback_contract_*_full122*_probe/`; the next non-tuning target is a state-consistent delayed visual constraint, not more source-stamp switching.
