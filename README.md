...
- Latest 2026-05-28 late visual/SE3 linearization-reference work: pending visual alignment and SE3 photometric factors now snapshot the reference pose available when the factor is produced, and marginalized late-prior conversion preserves that explicit reference instead of implicitly relinearizing against the current marginalized-state reference. This is a ROS2 callback/window-lifetime semantics fix rather than a scalar tune. The deterministic `sliding_window_schur_marginalization_probe` now includes `late_visual_reference_pose_probe`, where aligned explicit references produce zero cost and shifted explicit references produce nonzero visual/SE3 costs (`0.0249999` / `0.0262499`), proving the late-prior path honors the original factor reference. Full122 promotion still requires a replay against the accepted production preset; this is not a 100% paper-parity claim.
