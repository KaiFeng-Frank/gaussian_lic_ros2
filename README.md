...
- Current 100% paper/super-paper completion audit: `scripts/check_paper_completion.py` reports `INCOMPLETE` with `20` blockers in `docs/paper_completion_report.md`. The FAST-LIVO2 `CBD_Building_01` native tracker evidence is now sponsored by the hash-locked Coco-LIC2 cm-scale reference-parity gate; the remaining blockers are evidence-quality gaps on the non-CBD continuous-time tracker entries, which are still liveness-only instead of reference-trajectory RMSE/coverage parity, plus the README's explicit non-100% completion declarations.
