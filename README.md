...
- The experimental persistent point-map path behind `enable_persistent_point_map` now feeds a coupled 3D continuous-time point-to-point residual instead of three independent axis-plane residuals. This fixes the residual/Jacobian interface needed by Gaussian-map anchors, but it remains disabled as a parity default until the next CBD long-window sweep proves it reduces RMSE.
