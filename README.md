...
- Native reports now include a `visual_factor_continuity` block derived from the tracking-status bins. It records per-bin visual/SE3 factor deltas together with rendered/observed/depth miss and stale deltas, visual pair processed/duplicate deltas, SE3 valid/rejected batch deltas, and saturated-alignment deltas, so future full-window runs can distinguish transport/freshness loss from duplicate-pair suppression or visual quality rejection before another production-preset candidate is considered.
