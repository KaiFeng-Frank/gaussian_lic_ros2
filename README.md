...
- Latest 2026-05-28 rendered source-pose feedback contract: typed `RenderedFeedback` now carries `geometry_msgs/Pose source_pose`, the exact mapper camera pose used to render the feedback image. The tracker records that source pose in `VisualFrame` metadata and can use it under default-off `enable_rendered_feedback_source_pose_reference` as the visual/SE3 linearization reference instead of reconstructing the reference from the current active window. This targets a ROS2 process-boundary semantic gap: rendered residuals must be owned by the pose that produced the rendered image, not a later approximated sliding-window state. The production preset keeps the path disabled until full-window evidence beats the accepted gate. The 12 s proof at `results/fastlivo2/CBD_Building_01_rendered_source_pose_reference_12s_probe/native_tracking_report.json` passes with `98` typed rendered feedbacks, `98/98` visual/SE3 factors, `196` source-pose reference factors, zero invalid source poses, zero source-index gaps/duplicates, and `rendered_feedback_source_pose_reference_enabled=true`.
