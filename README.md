...
- Full Coco-LIC2-grade frontend/tracking algorithm: production sliding-window joint BA, analytic LIO/VIO Jacobians, robust IMU bias observability from LIO/VIO factors, and production-scale marginalization policy.
...
- The native tracker has timestamp-safe trajectory/IMU primitives, IMU history interpolation, raw-sample IMU preintegration/reintegration, IMU bias continuity residuals, dense marginalization-prior anchoring, reusable normal-equation linearization, Schur-complement retained-window dense priors, per-point LiDAR deskew, direct LiDAR/Gaussian-map point-to-point window factors, chunk-complete Gaussian snapshot caching, an optional sliding-window optimizer foundation, direct visual residual/alignment, and optional visual-alignment window factors, but it is not yet the full continuous-time Coco-LIC2 production BA stack.
- VIO pose Jacobians, robust IMU bias observability from LIO/VIO factors, analytic LIO/VIO Jacobians, and full joint BA remain to be ported beyond the current deskewed bounded 6-DoF LiDAR correction and finite-difference IMU-window foundation.
...
Expected local result is `16 tests, 0 errors, 0 failures, 0 skipped`.
...
These profiles currently cover the native mapper surface: topic names, QoS, synchronization, upstream camera intrinsics, image size, depth-completion toggles, Gaussian initialization/map-growth parameters, upstream optimizer/loss/exposure parameter names, output topics, and active profile labels. The native tracking launch also exposes per-point LiDAR deskew controls for common `offset_time`, `time`, `timestamp`, and `t` fields while preserving signed-nanosecond estimator math. The optional sliding-window optimizer is available behind `enable_sliding_window_optimizer:=true` for IMU-factor development, with reusable normal-equation linearization, dense prior anchoring, and Schur-complement retained-window priors in place. Full real-dataset frontend parity still needs Coco-LIC2-grade production BA, VIO Jacobians, and robust IMU bias optimization.
