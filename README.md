...
- `scripts/check_strict_parity_matrix.py` for the final full-dataset release gate. It currently reports `required=7/11`: FAST-LIVO2 mapper strict parity, FAST-LIVO hku1/hku2/Visual_Challenge/LiDAR_Degenerate mapper strict parity, M2DGR room_01 mapper strict parity, and CBD native BA runtime health are proven; native reference trajectory parity plus remaining M2DGR/MCD/R3LIVE full-sequence strict reports are still missing.
...
| Full-dataset strict parity matrix | Executable gate is in place; current status is incomplete at `7/11` required evidence items, with FAST-LIVO, FAST-LIVO2, and M2DGR covered by required mapper/native evidence |
...
Current local matrix status: `required=7/11`, `covered_profiles=fastlivo,fastlivo2,m2dgr`.
...
mapper-contract/CUDA strict parity, M2DGR room_01 mapper-contract/CUDA strict
parity, and the 120s CBD native visual/SE3 BA health report. Missing required
items are native reference trajectory parity for CBD, M2DGR room_02/room_03
strict parity, and full-sequence strict artifacts for MCD and R3LIVE.
