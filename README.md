...
- Stage-specific velocity step-guard scales are now exposed as default-off controls: `pre_lio_tracking_step_guard_velocity_scale` and `post_ba_tracking_step_guard_velocity_scale` can release only one guard stage while preserving the other. A 12 s CBD Gaussian-feedback smoke at `results/fastlivo2/CBD_Building_01_gaussian_feedback_pre_lio_velocity_scale_12s_smoke/native_tracking_report.json` proves the new pre-LIO-only path is live (`pre_lio=1.0`, `post_ba=0.0`, hard velocity cap `0.6 m/s`), but it records RMSE `0.168 m` and `581%` short-window path drift, so it remains a diagnostic surface rather than a production preset.
