...
- Latest 2026-05-23 estimator observability reporting: `run_native_tracking_bag_report.sh` now emits an `estimator_observability_continuity` section that bins normal-equation min/max singular values and condition number, dense-prior singular values, gyro/accel bias norms/components, bias-observability scalars, and IMU/visual/SE3/LiDAR/smoothness factor deltas. The 12 s proof at `results/fastlivo2/CBD_Building_01_observability_report_12s_rerun_probe/native_tracking_report.json` passes with 8 finalized bins, max normal-equation condition `5.535e8`, max gyro/accel bias norms `0.01345/0.20548`, and worst-information/worst-bias bin summaries. This is a diagnostic/gate upgrade for the remaining long-window frontend gap, not a tuning change.
