...
- Latest 2026-05-28 image-pose rendered-feedback source work: `mapping_node` now has a default-off `rendered_feedback_source_stream:=image_pose` path that owns rendered feedback from independent image and pose queues instead of from the point-cloud-aligned mapper frame. This is a ROS2 transport/ownership fix, not a BA scalar tune, and the production preset stays on `aligned_frame`. The 12 s proof at `results/fastlivo2/CBD_Building_01_image_pose_feedback_independent_queue_12s_probe/native_tracking_report.json` passes with `110` mapper images, `108` image-pose feedback candidates/published, zero feedback queue drops, `107/107` visual/SE3 factors, and only `7` pointcloud-anchor image-too-new drops, proving point-cloud alignment no longer gates feedback production. The 60 s replay at `results/fastlivo2/CBD_Building_01_image_pose_feedback_independent_queue_60s_probe/native_tracking_report.json` publishes `492` image-pose feedbacks from `493` mapper images but still misses the requested `500` visual/SE3 drain, while reliable mapper image QoS is explicitly rejected by `results/fastlivo2/CBD_Building_01_image_pose_feedback_reliable_image_qos_12s_probe/native_tracking_report.json` (`64` mapper images and `62/105` visual/SE3 factors). The full122 image-pose plus visual-marginalization-prior replay at `results/fastlivo2/CBD_Building_01_image_pose_feedback_marg_prior_full122_drain1015_probe/native_tracking_report.json` is also rejected: source ids stay clean and stale drops stay zero, but the run reaches only `1010/1015` visual and `1007/1015` SE3 factors and regresses to `1.598 m` RMSE with `5.64%` path drift. The remaining gap is mapper image delivery/long-window observation coupling, not a reason to flip high-rate images to reliable QoS or promote this path.
