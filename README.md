...
- Latest 2026-05-28 continuous-time acceleration-prior hookup: `TrajectoryEstimator` and `ContinuousTimeSlidingWindowEstimator` now expose a world-frame translational acceleration prior, carry it through signed-nanosecond buffering, include it in position Schur marginalization, and report factor/cost counts in the native diagnostic series. `continuous_time_node` can derive default-off acceleration priors from LiDAR scan-to-map or scan-to-scan velocity deltas, records target acceleration magnitudes, and exposes default-off physical clamping for LiDAR-pose-derived acceleration targets; `continuous_time_native_reference_parity.sh` forwards/archives the new controls. This is a structural trajectory-curvature residual aimed at the long-window shape failure, not a promoted tuning preset; the deterministic CTest probe verifies the residual is consumed and pulls spline curvature back to truth.
