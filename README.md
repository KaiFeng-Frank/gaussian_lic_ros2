...
These profiles currently cover the native mapper surface: topic names, QoS, synchronization, upstream camera intrinsics, image size, depth-completion toggles, Gaussian initialization/map-growth parameters, upstream optimizer/loss/exposure parameter names, output topics, and active profile labels. The native tracking launch also exposes sensor QoS controls, keeps high-rate tracking inputs at bounded `best_effort` by default, exposes per-point LiDAR deskew controls for common `offset_time`, `time`, `timestamp`, and `t` fields while preserving signed-nanosecond estimator math, and serializes tracking callbacks by default so a future multi-threaded executor cannot concurrently mutate IMU/LiDAR/image estimator state. The optional sliding-window optimizer is available behind `enable_sliding_window_optimizer:=true` for IMU-factor development, with reusable normal-equation linearization, dense prior anchoring, Schur-complement retained-window priors with rank/singular-value health, bias observability status, per-correspondence robust LiDAR/Gaussian-map weights, LiDAR point-to-plane factors, analytic geometric Jacobians, robust analytic SE3 camera photometric normal equations/window factors from rendered/current/depth images, runtime status for 2-DoF and SE3 photometric linearization quality, and subpixel visual alignment in place. Full real-dataset frontend parity still needs Coco-LIC2-grade production BA and production-scale marginalization policy.
