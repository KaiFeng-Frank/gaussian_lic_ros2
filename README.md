...
- LiDAR scan-to-map angular-velocity priors are wired on the continuous-time SO(3) derivative. CBD 12 s sweeps first kept the factor diagnostic-only, but the 10 Hz output trajectory stats exposed excessive long-window angular speed; a 60 s sweep now makes `lidar_pose_prior_angular_velocity_weight=0.1` the native parity preset because it reduces path drift from `264.75%` to `184.79%` while lowering p95 speed and angular speed. The standalone node's generic default remains `0.0` until the setting is validated across every dataset profile.
...
- Full-data CBD continuous-time evidence is now explicit: `results/fastlivo2/CBD_Building_01_ct_stamp_driven_full_output10hz/native_tracking_report.json` replays the local 118.709 s frontend-raw bag with `PLAYBACK_DURATION=full`, captures 1184 odometry poses, matches 1173 reference poses (`95.29%` coverage), and records RMSE `3.665 m`, mean `3.303 m`, max `8.128 m`, and path drift `64.78%`. This resolves the "data first" blocker for this gate and confirms the remaining strict blocker is long-window BA trajectory shape, not missing CBD input data.
