...
- Latest 2026-05-23 structural tracker work: visual-alignment and SE3 photometric sliding-window factors can now attach to the two bracketing continuous-time states through default-off `enable_visual_factor_time_interpolation` instead of snapping every delayed observation to one discrete state. The legacy production preset keeps this off, so zero numeric-Jacobian fallback remains enforced there; when the diagnostic is enabled the native report allows and records the expected local numeric blocks. The CBD 12 s proof at `results/fastlivo2/CBD_Building_01_visual_time_interpolation_12s_maxinvalid1_probe/native_tracking_report.json` passes with 99 poses, 73 visual factors, 73 SE3 factors, 36 interpolated visual factors, 36 interpolated SE3 factors, dense prior rank/cols 180/180, and zero orphan factors. This is a structural continuous-time semantics fix, not a parameter-tuning result, and it still needs full-window RMSE evidence before promotion.
