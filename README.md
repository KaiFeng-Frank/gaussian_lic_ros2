...
- The sliding-window Schur marginalization prior weight is now exposed through launch and `run_native_tracking_bag_report.sh --sliding-window-marginalization-prior-weight`, then archived in `native_tracking_report.json::gate_config`. A 12 s CBD smoke at `/tmp/gaussian_feedback_marginalization_weight_12s_smoke/native_tracking_report.json` proves the value is live (`0.5`) with Gaussian feedback enabled and the relative-translation preset active. This is a sweep surface for the full120 late-window path-shortening failure, not a default parity claim until a full120 report beats the current checkpoint.
