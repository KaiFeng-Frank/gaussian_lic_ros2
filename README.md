...
- Native tracking package with signed-nanosecond trajectory and IMU primitives, IMU history interpolation, raw-sample IMU preintegration/reintegration, IMU bias continuity inside the optional sliding window, per-point LiDAR deskew, bounded 6-DoF LiDAR residual correction, visual residual comparison against mapper renders, GaussianMap snapshot subscription, odometry/path/optional TF publication, and deterministic probes.
...
- Full Coco-LIC2-grade frontend/tracking algorithm: production sliding-window joint BA, VIO Jacobians, robust IMU bias observability from LIO/VIO factors, and Schur/marginalization.
...
- The native tracker has timestamp-safe trajectory/IMU primitives, IMU history interpolation, raw-sample IMU preintegration/reintegration, IMU bias continuity residuals, per-point LiDAR deskew, an optional sliding-window optimizer foundation, and initial LiDAR/visual/Gaussian snapshot factors, but it is not yet the full continuous-time Coco-LIC2 production BA stack.
- VIO residual Jacobians, robust IMU bias observability from LIO/VIO factors, Schur/marginalization, and full joint BA remain to be ported beyond the current deskewed bounded 6-DoF LiDAR correction and finite-difference IMU-window foundation.
