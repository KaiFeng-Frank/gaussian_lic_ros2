...
- Latest short CBD continuous-time smoke, 2026-05-12: the pre-guard persistent-plane path produced a 4 s prior-seeded RMSE of about `144103 m`; after solve-step rejection the same 4 s prior-seeded smoke is finite with `41` TUM poses, `34` reference matches, RMSE `2.725 m`, and max error `4.591 m` at `results/fastlivo2/CBD_Building_01_continuous_time_solver_guard_prior_smoke_4s/trajectory_compare_report.json`. The no-prior variant is finite with RMSE `11.104 m`. These are hardening checkpoints, not paper parity.
