...
- Native tracking reference comparisons now archive 8 time-ordered error bins by default through `run_native_tracking_bag_report.sh --reference-error-bin-count`, so each report shows where late-window drift starts instead of requiring a manual `trajectory_compare.py` rerun. The latest 60 s CBD factor-weight probes keep the blocker in trajectory-shape/global coupling: native LiDAR plane weight `2.0` gives a small improvement to `0.792 m` RMSE, point weight `2.0` gives `0.822 m`, point+plane `2.0` regresses to `1.201 m`, the `se3=0.25` reproduction regresses to `0.853 m`, adaptive `0.35 m/s` velocity guard regresses to `0.855 m` with `39.9%` path drift, and the position-velocity smoothness coverage rerun regresses to `0.828 m`.
