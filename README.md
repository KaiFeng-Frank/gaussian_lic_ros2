...
- Latest 2026-05-28 continuous-time SO(3) marginalization hookup: the same optional sliding-window path now also preserves rotation information when old B-spline knots leave the window. `TrajectoryEstimator` accepts dense SO(3) tangent-space priors through `add_dense_orientation_prior_factor`, and `ContinuousTimeSlidingWindowEstimator` Schur-linearizes active orientation priors, second-difference rotation smoothness, and retained dense orientation priors into a reinjected dense orientation prior. Runtime diagnostics expose total/last-step orientation marginalization prior counts and rows. The new deterministic checks cover direct dense orientation anchoring plus streaming generation/reinjection; `colcon test --packages-select gaussian_lic_tracking` passes. The 12 s real-bag liveness proof at `results/fastlivo2/CBD_Building_01_ct_dense_orientation_marg_12s_probe/native_tracking_report.json` records `170` position marginalization priors / `1272` rows and `197` orientation marginalization priors / `1437` rows with all `52` solver steps accepted, proving the new path executes on CBD replay. It is not promoted because short-window trajectory compare remains non-parity (`0.473 m` RMSE, `36.56x` path-relative drift on only `8.85%` full-reference coverage). This is a structural long-window BA information-retention fix, not a parameter-tuning promotion, and it still needs full CBD replay before it can replace the accepted production preset.
