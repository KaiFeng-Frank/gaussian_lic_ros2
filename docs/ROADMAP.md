...
- [ ] Strict `CBD_Building_01` ROS2 current report with trajectory coverage, render pairs, PSNR/SSIM/LPIPS, and Chamfer within the paper gate.
  - 2026-05-05 local strict run with upstream-random optimizer-frame sampling: trajectory gate passes with 1167/1186 matched poses and 98.40% coverage after the 60 second post-playback settle window.
  - 2026-05-05 local strict run: ROS2 novel PSNR 12.45 dB vs ROS1 12.70 dB passes, ROS2 novel LPIPS 0.747 vs ROS1 0.751 passes, but ROS2 novel SSIM 0.332 vs ROS1 0.364 still fails with an 8.84% regression.
  - 2026-05-05 local strict run: after full-sequence even report sampling, the 64-pair ROS1-vs-ROS2 render summary is mean PSNR 20.15 dB and mean SSIM 0.664 with upstream-style alpha-hole extension filtering, upstream-random optimizer-frame sampling, disabled split/clone densification, disabled opacity resets, and zero optimizer OOMs.
  - 2026-05-05 local strict run: point-cloud parity still fails but is close: centroid drift 0.444 m, nearest mean 0.1022 m, nearest RMSE 0.1441 m, unmatched ratio 2.17%, declared point-count ratio 0.610, and diagnostic loaded sample ratio 0.975.
  - 2026-05-05 local strict run: remaining blockers are residual SSIM parity and point-cloud centroid/Chamfer mean parity, not missing render pairs, OOM, ROS2-only densification, opacity reset, sorted optimizer-frame sampling, or disabled CUDA runtime.
