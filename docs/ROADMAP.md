...
- [x] Add an executable full-dataset strict parity evidence matrix.
  ...
  - Current local matrix status is `required=5/10`, `covered_profiles=fastlivo,fastlivo2`: the FAST-LIVO2 `CBD_Building_01` mapper-contract/CUDA strict report, FAST-LIVO hku1/hku2/Visual_Challenge mapper-contract/CUDA strict reports, and CBD native visual/SE3 BA health report pass, while CBD native reference trajectory parity plus FAST-LIVO LiDAR_Degenerate and the remaining M2DGR/MCD/R3LIVE full-sequence strict artifacts are still missing.
