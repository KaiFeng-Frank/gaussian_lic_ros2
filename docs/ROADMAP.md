...
- [x] Add native continuous-time 3D point-to-point residuals for persistent/Gaussian map anchors.
  ...
  - CTest now covers both the batch estimator and online sliding-window consumption path. This is the correct residual interface for global Gaussian-map coupling, but it is not marked as tracker parity until a long-window CBD run beats the current ~1.8 m RMSE checkpoint.
