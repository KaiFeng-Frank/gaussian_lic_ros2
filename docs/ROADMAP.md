...
- [ ] Strict `CBD_Building_01` ROS2 current report with trajectory coverage, render pairs, PSNR/SSIM/LPIPS, and Chamfer within the paper gate.
  ...
  - 2026-05-05 local strict run: the 64-pair ROS1-vs-ROS2 render summary is mean PSNR 22.67 dB and mean SSIM 0.954 with upstream-style alpha-hole extension filtering, disabled split/clone densification, disabled opacity resets, and zero optimizer OOMs.
  - 2026-05-05 local strict run: point-cloud parity still fails but is close to the Chamfer thresholds except centroid drift: centroid drift 1.07 m, nearest mean 0.1068 m, nearest RMSE 0.1521 m, and unmatched ratio 2.88%.
