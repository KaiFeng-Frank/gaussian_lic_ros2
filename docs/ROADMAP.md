...
- [x] Add an executable full-dataset strict parity evidence matrix.
  ...
  - 2026-05-13 scan-to-scan target-quality gates: the relative-motion path now reports raw target translation, target/prediction ratio, small-target counts, prediction-fallback counts, and skipped translation priors. `lidar_scan_to_scan_min_target_prediction_ratio`, `lidar_scan_to_scan_min_target_translation_m`, `lidar_scan_to_scan_use_prediction_on_small_target`, and `lidar_scan_to_scan_skip_translation_priors_on_small_target` are exposed through the native parity script. CBD 12 s probes show the diagnosis clearly: prediction fallback at ratio `0.5` fires 39 times and lifts the target path, but optimized path remains `0.045 m`; skip-small-translation at ratio `0.95` skips 84 translation priors but still records path `0.053 m`; stronger `pos5/vel1` weights reach path `0.338 m`, while `pos50/vel10` overdrives to `2.412 m` and worsens RMSE. The implementation prevents silent target collapse and gives reproducible evidence, but full parity still needs global visual/Gaussian-map trajectory-shape constraints.
  - 2026-05-13 Gaussian snapshot LiDAR anchors: `continuous_time_node` now has a default-off mapper-to-tracker reverse channel that subscribes to chunked `GaussianArray` snapshots, waits for complete snapshots, voxel-indexes opacity-gated Gaussian centers, and turns LiDAR scan points into robust point-to-point Ceres residuals against the Gaussian map. `continuous_time_native_reference_parity.sh` forwards and archives the Gaussian snapshot topic, weight, nearest-distance gate, min-opacity gate, frame/map subsampling, and max-correspondence controls. `run_native_tracking_bag_report.sh` now also has `--enable-gaussian-map-feedback` and `--require-gaussian-snapshot`, which start mapper Torch Gaussian init/extend with `GaussianArray` publication and make the health report fail if tracking never receives a complete map snapshot. The first CBD probe exposed the ROS2 chunked-map QoS failure (`1/5` chunks with depth 1); after adding mapper `gaussian_map_qos_depth` plus tracking/continuous-time `gaussian_snapshot_qos_depth` and using depth 128, `results/fastlivo2/CBD_Building_01_gaussian_map_feedback_qos128_probe_6s/native_tracking_report.json` passes with `17716` Gaussians, `5/5` chunks, and 18 point-factor batches. This closes the missing online Gaussian-map global-anchor plumbing; the strict blocker remains proving that full mapper-feedback long-window runs improve RMSE to the paper gate.
