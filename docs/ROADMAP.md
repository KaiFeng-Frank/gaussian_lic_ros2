...
- [x] Add an executable full-dataset strict parity evidence matrix.
  ...
  - Current local matrix status is `required=7/11`, `covered_profiles=fastlivo,fastlivo2,m2dgr`: the FAST-LIVO2 `CBD_Building_01` mapper-contract/CUDA strict report, FAST-LIVO hku1/hku2/Visual_Challenge/LiDAR_Degenerate mapper-contract/CUDA strict reports, M2DGR room_01 mapper-contract/CUDA strict report, and CBD native visual/SE3 BA health report pass, while CBD native reference trajectory parity plus M2DGR room_02/room_03 and the remaining MCD/R3LIVE full-sequence strict artifacts are still missing.
