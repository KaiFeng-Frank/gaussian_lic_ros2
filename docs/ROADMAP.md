...
- [x] Add an executable full-dataset strict parity evidence matrix.
  ...
  - Current local matrix status is `required=9/12`, `covered_profiles=fastlivo,fastlivo2,m2dgr`: the FAST-LIVO2 `CBD_Building_01` mapper-contract/CUDA strict report, FAST-LIVO hku1/hku2/Visual_Challenge/LiDAR_Degenerate mapper-contract/CUDA strict reports, M2DGR room_01/room_02/room_03 mapper-contract/CUDA strict reports, and CBD native visual/SE3 BA health report pass, while CBD native reference trajectory parity plus the remaining MCD/R3LIVE full-sequence strict artifacts are still missing.
