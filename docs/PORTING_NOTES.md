...
The CUDA rasterizer, fused SSIM loss, visibility-masked SparseGaussianAdam, densification/pruning, and strict CUDA rendered preview are ported behind the Torch/CUDA build profile. The native tracking package now provides timestamp-safe trajectory/IMU primitives, signed-nanosecond IMU history interpolation, raw-sample IMU preintegration/reintegration, IMU bias continuity residuals and marginalization-prior anchoring in the optional sliding window, per-point LiDAR deskew, native odometry/path/TF publication, bounded 6-DoF LiDAR residual correction, direct LiDAR point-to-point window factors, a visual residual path that compares mapper rendered images with incoming camera frames, and chunk-complete `GaussianArray` snapshot caching for the mapper-to-tracker reverse channel. Full Coco-LIC2-grade production joint BA is still separate from this mapper backend work.
